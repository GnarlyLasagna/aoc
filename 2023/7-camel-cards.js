...
const handStrengths = ["high card","one pair","two pair","three of a kind","full house","four of a kind","five of a kind"]

    arr.forEach(element =>{
        Object.entries(element).forEach(([key, value]) => {
        console.log(`${key} ${value}`)
        const curr = handStrengths.indexOf(element.hand)
        if(key !== "winnings" && key !== "hand" && key !== "highest"){


        if(value === 1 && curr < 0 ){
            element.hand = handStrengths[0]
        } 
        else if((value === 3 && curr === 1 ) || (value === 2 && curr === 3)){
            element.hand = handStrengths[4]
        } 
        else if(value === 2 && curr < 1 ){
            element.hand = handStrengths[1]
        } 
        else if(value === 2 && curr === 1 ){
            element.hand = handStrengths[2]
        } 
        else if(value === 3 && curr < 2 ){
            element.hand = handStrengths[3]
        } 
       
        else if(value === 4 && curr < 4 ){
            element.hand = handStrengths[5]
        } 
        else if(value === 5 && curr < 5 ){
            element.hand = handStrengths[6]
        }
        }
        })
    })
     arr.sort((a,b)=>{ 
         for(i=0;i<a.strengths.length;i++){
             if(a.strengths[i]!==b.strengths[i]){
                return a.strengths[i]-b.strengths[i]
             }
    console.log("testing stuff ",i)}
    })
    arr.sort((a,b)=>{ 
    return handStrengths.indexOf(a.hand) - handStrengths.indexOf(b.hand)
    })
    console.log(arr)
    return arr
}
...
hand.forEach(element => {
    ...
    const temp = {}
    const strengthsTranslation = []
    const cards = element.split(" ")[0]
    const points = element.split(" ")[1]
    temp.original = cards
    temp.winnings = points

    for(i=0;i<cards.length;i++){
        strengthsTranslation.push(cardStrengths.indexOf(cards[i]))
        temp.strengths = strengthsTranslation
    //console.log(cards[i])
        if(temp[cards[i]]){
        temp[cards[i]]++
        }else{
    ...
    processedCards.push(temp)
})
readCards(processedCards)
let ans = 0

processedCards.forEach((element,index)  =>{
ans += Number(element.winnings) * Number(index + 1)
console.log("ans ",ans)    
})
...
// sortCards(testInput)
// sortCardsTwo(testInput)

const realInput = `342QK 491
36QAT 619
85663 606
33K3A 23
K7775 814
