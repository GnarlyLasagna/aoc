...
function colorTracker(input){
    const ansKey = {'red':12,'gre':13,'blu':14}
    const arr = input.split('\n')
    let ans = 0

        for (i=0; i<arr.length; i++){
            let gamesArray = arr[i].split(" ")
            let gameNum = gamesArray[1].slice(0,gamesArray[1].length -1)
            let sumMap = {'red':0,'gre':0,'blu':0}

        for(j=2; j<gamesArray.length; j++){
            let color = gamesArray[j+1]
            let count = gamesArray[j]
            sumMap[color.slice(0,3)]+=Number(count)
            j++
            if(color[color.length-1] === ";"){
                if(sumMap['red']>ansKey['red'] || sumMap['gre']>ansKey['gre'] || sumMap['blu']>ansKey['blu'] ) {
                    break
                }
            sumMap = {'red':0,'gre':0,'blu':0}
            }
        }
        if(sumMap['red']<=ansKey['red'] && sumMap['gre']<=ansKey['gre'] && sumMap['blu']<=ansKey['blu'] ) {
            ans += Number(gameNum)
...
function colorTrackerPartTwo(input){
    const arr = input.split('\n')
    let ans = 0
        ...
        for (i=0; i<arr.length; i++){
            let gamesArray = arr[i].split(" ")
            let sumMap = {'red':0,'gre':0,'blu':0}
            let lowestSumMap = {'red':0,'gre':0,'blu':0}
        ...
        for(j=2; j<gamesArray.length; j++){
            ...
            sumMap[color.slice(0,3)]+=Number(count)
            j++
            if(sumMap['red']>lowestSumMap['red']) {
                lowestSumMap['red'] = sumMap['red']
            }
            if(sumMap['gre']>lowestSumMap['gre']) {
                lowestSumMap['gre'] = sumMap['gre']
            }
            if(sumMap['blu']>lowestSumMap['blu']) {
                lowestSumMap['blu'] = sumMap['blu']
            }
            if(color[color.length-1] === ";"){
                sumMap = {'red':0,'gre':0,'blu':0}
            }
        }
        ans+= Number(lowestSumMap['red']*lowestSumMap['gre']*lowestSumMap['blu']) 
    }
    console.log(ans)
    return ans
}

// colorTracker(testInput)
// colorTrackerPartTwo(testInput)

const realInput = `Game 1: 1 red, 5 blue, 1 green; 16 blue, 3 red; 6 blue, 5 red; 4 red, 7 blue, 1 green
...
colorTracker(realInput)

colorTrackerPartTwo(realInput)
